...
- Rate-limit counters are durable in SQLite. Chat, auth, sharing, uploads, voice, images, and global writes are bounded.
...
- Mount `/app/data` on encrypted persistent storage and back it up.
...
- Keep `WORKERS=1` and one application replica until SQLite and stream control are replaced with shared services.
...
SQLite with WAL is suitable for a small single-instance deployment. Horizontal scaling requires a server database, distributed rate limiting, shared stream state, and external object storage.
